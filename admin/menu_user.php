<div id="nav">
	  <ul>
	 
       <li class="upp"><a href="home.php">Home</a></li>
<!-- <li class="upp"><a href="settings.php">Settings</a></li> -->
            
 <li class="upp">Project
     <ul>
      <li>› <a href="project_add.php">Add Project</a></li> 
       <li>› <a href="project_list.php">Project List</a></li>
     </ul>
</li>

 <li class="upp">Users
     <ul>
       <li>› <a href="user_password_change.php">Change Password</a></li>   
       <li>› <a href="image_update.php">Upload Picture</a></li>   
     </ul>
</li>
 			<li class="upp"><a href="#">Report</a>
	      <ul>
          <li>› <a href="summery.php">Summary</a></li>   

          </ul>
          </li>          
    <li class="upp"><a href="#">Help</a></li>
  <li class="upp"><a href="logout.php">Logout</a></li>
 </ul>
</div>
    <!--
    MD jahidur Rahman
    jahid_iubat@yahoo.com
    -->